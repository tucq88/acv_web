...
function wp_logo_slider() {
	echo '<div class="wrap">';
        ...
        // comment add setting manager plugin by hoanx
...
function logo_slider($args = array(), $content = null) {
    ...
    foreach ($data_chunks as $data_chunk) {
        ...
        foreach($data_chunk as $data) {
            echo '<div class="col-md-2'.($i>3 ? ' hidden-sm' : ' col-sm-3').($i>2 ? ' hidden-xs' : ' col-xs-4').'">';
            if($data['image_links_to'])
                echo '<a href="'.$data['image_links_to'].'">';
            echo '<img src="'.$data['file_url'].'" class="img-responsive" alt="" />';
...
// comment add style plugin by hoanx
...
// comment add js plugin by hoanx
